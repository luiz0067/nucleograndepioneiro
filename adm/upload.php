...
$result=null;
if (($_GET["id"]!=null)){
	$sql	= "SELECT id,imagem,nome,link FROM patrocinio where (id=0".$_GET["id"].")";
			...
			else
			{
				...
				echo "<a href=\"..\\albun_de_fotos\\".$imagem."\">".$imagem."</a><br>";
					if ($_POST['acao']=='excluir'){
						...
						$sql = 'delete FROM patrocinio where id='.$_POST["id"];
						//echo $sql;
						mysql_query($sql, $link);
					}
					else if ($_POST ['acao']=='alterar'){
						delete_file($folder . $imagem _antiga);
						$sql = "update patrocinio set imagem='".$_POST["imagem"]."', nome='".$_POST["nome"]."',link='".$_POST["link"]."' where (id=".$_POST["id"].");";
						//echo $sql;
						mysql_query($sql, $link);
					}
